...
// 获取选中的芯片类型
function getSelectedChipset() {
    const chipsetRadios = document.getElementsByName('chip'); // 修改为'chip'以匹配HTML
    for (const radio of chipsetRadios) {
        if (radio.checked) {
            selectedFiles.chipset = radio.value;
            return radio.value;
        }
    }
    return 'ascend';
...
function checkAllSelected() {
    ...
    const isChipsetSelected = getSelectedChipset() !== null;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 绑定单选按钮事件
    document.querySelectorAll('input[name="chip"], input[name="task"]').forEach(radio => {
...
window.serverInference = async function() {
    ...
    try {
        ...
        if (selectedFiles.task === 'classification') {
            ...
            groundResult.textContent = logContent;
