...
window.selectFile = async function(type) {
    ...
    if (paths && paths.length > 0) {
        ...
        selectedFiles.model_path = filePath;
    }
};
...
window.deployModel = async function() {
    ...
    try {
        ...
        // 保存配置文件
        const savePath2 = await window.parent.electron.saveJson(config2, `page2_convert_${selectedFiles.type}.json`);
        ...
        // 等待并检查服务是否可用
        const isServiceAvailable = await checkServiceAvailability('http://localhost:8082');
        ...
        modelDisplay.innerHTML = `
            <div style="position: relative;">
                ...
                <button 
                    ...
                    onclick="window.open('http://localhost:8082', '_blank', 'width=1200,height=800')"
                >
                    全屏查看
                </button>
            </div>
