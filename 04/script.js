...
window.AIInference = async function() {
    ...
    try {
        ...
        const fetchAI = async () => {
            if (Date.now() > endTime) return;
            try {
                const logContent = await window.parent.electron.readFile(`${selectedFiles.export_path}/infer_result.txt`);
...
window.dataVerify = async function() {
    ...
    try {
        ...
        const fetchCompare = async () => {
            if (Date.now() > endTime) return;
            try {
                const logContent = await window.parent.electron.readFile(`${selectedFiles.export_path}/result.txt`);
